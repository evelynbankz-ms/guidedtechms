/* ============================================================
   FILE: app/portal-subscriptions.js
   Page 2 — My Subscriptions
   Shows active subs or empty state + recommendations
   ============================================================ */
    ...
    <div class="sub-card">
      ...
      ${price ? `
        <div class="sub-card-price">
          $${esc(fmtPrice(price))} <span>/ ${esc(billing)}</span>
        </div>` : ""}
...
async function fetchUserSubscriptions(uid) {
  ...
  // Check service activations (individual services)
...
async function loadRecommendations() {
  ...
  try {
    ...
    console.error("Load rec services:", err);
  }
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const activeSection = document.getElementById("activeSubsSection");
  const emptySection  = document.getElementById("emptySubsSection");

  if (!user) {
    // Not signed in — show empty state with recommendations
    loadText.textContent = "Loading recommendations…";
    await loadRecommendations();
    loadText.textContent = "Sign in to view your subscriptions";
    return;
  }
  ...
  try {
    ...
    if (subs.length > 0) {
      // User HAS subscriptions — show them
      emptySection.style.display  = "none";
      activeSection.style.display = "block";

      const grid = document.getElementById("subCardsGrid");
      if (grid) grid.innerHTML = subs.map(s => renderSubCard(s)).join("");

      loadText.textContent = `${subs.length} active subscription${subs.length > 1 ? "s" : ""}`;
    } else {
      // User is signed in but has no subscriptions — empty state + recommendations
      loadText.textContent = "Loading recommendations…";
      await loadRecommendations();
      loadText.textContent = "No active subscriptions";
    }
  } catch (err) {
    console.error("Fetch subscriptions:", err);
    loadText.textContent = "Error loading subscriptions";
    await loadRecommendations();
  }
});
